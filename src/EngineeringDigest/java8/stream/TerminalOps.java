...
public class TerminalOps {
    ...
    public static void main(String[] args) {
        ...
        // Accumulator = the running result (current value) while processing data
        // The accumulator is the intermediate result, and the accumulator function defines
        // how each new element is combined with that result.
        Optional<Integer> optionalInteger = list.stream().reduce(Integer::sum);
        System.out.println(optionalInteger.get());

        List<Integer> listAcc = List.of(1, 2, 3, 4);
        int result = listAcc.stream().reduce(0, (a, b) -> a + b);
        // Accumulator is the variable that stores the intermediate result during reduction.
        // a → accumulator (running result)
        // b → current element
        // Start → accumulator = 0
        // Step 1: 0 + 1 = 1   → accumulator = 1
        // Step 2: 1 + 2 = 3   → accumulator = 3
        // Step 3: 3 + 3 = 6   → accumulator = 6
        // Step 4: 6 + 4 = 10  → accumulator = 10
        ...
        // 6. findFirst, findAny
        // findFirst → returns the FIRST element (respects order)
        // findAny   → returns ANY element (may be random, faster in parallel)
