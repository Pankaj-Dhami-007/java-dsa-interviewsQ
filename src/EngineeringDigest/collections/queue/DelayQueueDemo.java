...
//DelayQueue is a time-based blocking queue where elements become available only after their delay
// expires, making it ideal for scheduling and retry systems.
...
// Definition:
// A thread-safe, unbounded BlockingQueue where elements
// can only be taken when their delay has expired

// Key Points:
// - Elements must implement Delayed interface
// - Ordered by remaining delay (earliest first)
// - Internally uses PriorityQueue (min-heap)

// Core Methods:
// put(e)  → inserts element (never blocks)
// take()  → blocks until delay expires
// poll()  → returns null if no expired element

// Use case:
// Task scheduling, retries, time-based execution